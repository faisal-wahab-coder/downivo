...
Flutter monorepo for Universal Downloader (Android + Web).
...
```bash
cd universal_downloader/apps/mobile
flutter pub get
flutter run
```
...
```bash
cd universal_downloader/apps/web
flutter pub get
flutter run -d chrome
```

Validate the monorepo: `bash scripts/ci.sh` from `universal_downloader/`.
