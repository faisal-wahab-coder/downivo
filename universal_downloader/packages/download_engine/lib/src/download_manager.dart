...
class DownloadManager {
  ...
  Future<void> _runDownload(DownloadTask task) async {
    ...
    try {
      ...
      // ignore: avoid_print
      print(
        'DL-START [${task.url}]: CDN=${Uri.tryParse(downloadUrl)?.host ?? downloadUrl}',
      );
      if (_isAbandoned(task.id)) return;
      ...
      // ignore: avoid_print
      print(
        'DL-ERROR [${task.url}]: DioException ${error.type} '
      ...
      // ignore: avoid_print
      print('DL-ERROR [${task.url}]: ${error.runtimeType}: $error');
