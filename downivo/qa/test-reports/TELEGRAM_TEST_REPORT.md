...
Telegram support is **implemented and tested** in the Downivo download engine. Public `t.me` / `telegram.me` / `tg://resolve` / CDN URLs are classified, normalized, and resolved through `TelegramResolver` into the **existing Download Engine**. There is no Telegram-specific downloader.
