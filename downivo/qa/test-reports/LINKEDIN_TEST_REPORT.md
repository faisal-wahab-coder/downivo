...
LinkedIn support is **implemented and tested** in the Downivo download engine. Before this phase LinkedIn was only a host label: `SocialPlatform.fromUri` matched `linkedin.com`, there was no `LinkedInUri`, no dedicated resolver, and generic Open Graph could treat a player page (`og:video` `text/html`) as a file.
