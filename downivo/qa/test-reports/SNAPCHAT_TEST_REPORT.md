...
Snapchat support is **implemented and tested** in the Downivo download engine. Public `snapchat.com` / `t.snapchat.com` / `story.snapchat.com` / `snapchat://` / CDN URLs are classified, normalized, and resolved through `SnapchatResolver` into the **existing Download Engine**. There is no Snapchat-specific downloader.
